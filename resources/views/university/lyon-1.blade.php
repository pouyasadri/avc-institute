...
@section('useful_links')
    <div class="sidebar-widget p-4 rounded-5 shadow-sm bg-white mb-4 border-0">
        ...
        <ul class="list-unstyled mb-0">
            <li class="mb-2">
                <a href="https://www.univ-lyon1.fr/" target="_blank" rel="noopener noreferrer" class="d-flex align-items-center text-decoration-none">
...
@section('university_content')
    ...
    @if(Lang::has('university/lyon-1.geo_capsule_text'))
        <div class="p-4 mb-4 rounded-4 bg-primary-subtle border border-primary-subtle" role="region" aria-label="{{ __('university/lyon-1.geo_capsule_title') }}">
            <div class="d-flex align-items-center gap-2 mb-2 text-primary fw-bold">
                <i class="bx bxs-check-shield fs-5"></i>
                <h3 class="h6 fw-bold mb-0 text-primary">{{ __('university/lyon-1.geo_capsule_title') }}</h3>
            </div>
            <p class="mb-0 text-dark small leading-relaxed">
                {{ __('university/lyon-1.geo_capsule_text') }}
            </p>
        </div>
    @endif
    ...
    @if(Lang::has('university/lyon-1.tuition_title'))
        <section class="mb-5 p-4 rounded-4 bg-light border border-secondary-subtle">
            <h3 class="h5 fw-bold mb-3 text-dark">
                <i class="bx bx-money text-primary me-2"></i>{{ __('university/lyon-1.tuition_title') }}
            </h3>
            <p class="text-muted small leading-relaxed mb-0">{{ __('university/lyon-1.tuition_content') }}</p>
        </section>
    @endif
    ...
    @if(Lang::has('university/lyon-1.comparison_title'))
        <section class="mb-5 p-4 rounded-4 bg-light border border-primary-subtle">
            <h3 class="h5 fw-bold mb-3 text-primary">
                <i class="bx bx-git-compare me-2"></i>{{ __('university/lyon-1.comparison_title') }}
            </h3>
            <p class="text-muted small leading-relaxed mb-3">{{ __('university/lyon-1.comparison_content') }}</p>
            <div class="row g-2 pt-2">
                <div class="col-md-4">
                    <a href="{{ url(app()->getLocale() . '/universities/lyon-2') }}" class="btn btn-outline-primary btn-sm rounded-pill w-100 d-flex align-items-center justify-content-center gap-1">
                        <i class="bx bx-palette"></i>
                        <span>دانشگاه لیون ۲ (هنر و علوم انسانی)</span>
                    </a>
                </div>
                <div class="col-md-4">
...
@push("json")
    @php
        ...
        $faqItems = __('university/lyon-1.faq_items');
