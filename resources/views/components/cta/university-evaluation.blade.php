...
@php
    ...
    $univLabel = $universitySlug ? ucwords(str_replace('-', ' ', $universitySlug)) : '';
    if ($currentLocale === 'fa') {
        $prefillDetails = 'درخواست ارزیابی شانس پذیرش و ویزا برای ' . ($univLabel ?: 'دانشگاه‌های فرانسه');
    } elseif ($currentLocale === 'fr') {
        $prefillDetails = 'Demande d\'évaluation d\'admissibilité et visa pour ' . ($univLabel ?: 'les universités françaises');
    } else {
        $prefillDetails = 'Admission and student visa evaluation request for ' . ($univLabel ?: 'French universities');
    }

    $consultUrl = url($currentLocale . '/consult?service=educational-counseling&details=' . urlencode($prefillDetails));
    $waPhone = '33768688326';
    $waMsg = __('cta.university.whatsapp_msg');
    $waUrl = 'https://wa.me/' . $waPhone . '?text=' . urlencode($waMsg);
@endphp

<aside class="cta-evaluation-banner my-5 p-4 p-md-5 rounded-4 border border-primary-subtle shadow-sm position-relative overflow-hidden"
       style="background: linear-gradient(135deg, #f8faff 0%, #edf3fc 100%);"
       aria-label="{{ __('cta.university.title') }}">
    <div class="row align-items-center g-4">
        <div class="col-lg-12">
            <div class="d-inline-flex align-items-center gap-2 px-3 py-1 mb-3 rounded-pill bg-primary-subtle text-primary fw-semibold small">
                ...
                <span>{{ __('cta.university.badge') }}</span>
            </div>

            <h3 class="h4 fw-bold text-dark mb-3">
                {{ __('cta.university.title') }}
            </h3>

            <p class="text-secondary mb-4 leading-relaxed">
                {{ __('cta.university.subtitle') }}
            </p>
            ...
            <div class="row g-2 mb-4">
                <div class="col-md-4">
                    <div class="d-flex align-items-start gap-2 small text-dark">
                        ...
                        <span>{{ __('cta.university.feature_1') }}</span>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="d-flex align-items-start gap-2 small text-dark">
                        <i class="bx bx-check-circle text-primary fs-5 mt-1 flex-shrink-0"></i>
                        <span>{{ __('cta.university.feature_2') }}</span>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="d-flex align-items-start gap-2 small text-dark">
                        <i class="bx bx-check-circle text-primary fs-5 mt-1 flex-shrink-0"></i>
                        <span>{{ __('cta.university.feature_3') }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="d-flex flex-wrap align-items-center gap-3 pt-2">
                <a href="{{ $consultUrl }}"
                   ...
                   title="{{ __('cta.university.primary_btn') }}">
                    <i class="bx bx-calendar-check fs-5"></i>
                    <span>{{ __('cta.university.primary_btn') }}</span>
                ...
                <a href="{{ $waUrl }}"
                   ...
                   title="{{ __('cta.university.whatsapp_btn') }}">
                    <i class="bx bxl-whatsapp fs-5"></i>
                    <span>{{ __('cta.university.whatsapp_btn') }}</span>
                </a>
            </div>
