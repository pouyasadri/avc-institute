<?php

return [
    // SEO Meta
    'title' => 'Votre Guide pour l\'Université Paris-Saclay | Opportunités 2026',
    'keywords' => 'Université Paris-Saclay, étudier en France 2026, classement Paris-Saclay 2026, vie étudiante France, recherche Paris, admission étudiants internationaux',
    'description' => 'Prêt à rejoindre un leader mondial de l\'innovation ? Découvrez l\'Université Paris-Saclay. Classée 13e mondiale (Shanghai 2026), elle offre un cadre chaleureux et dynamique pour votre réussite en 2026.',

    // Page Title Area
    'main_heading' => 'Bienvenue à l\'Université Paris-Saclay',
    'breadcrumb_home' => 'Accueil',
    'breadcrumb_universities' => 'Nos Universités Partenaires',
    'breadcrumb_current' => 'Université Paris-Saclay',

    // Sidebar
    'table_of_contents' => 'Au Sommaire',
    'contact_us' => 'Discutez avec notre Équipe',
    'consultation_request' => 'Préparons ensemble votre rentrée 2026',
    'useful_links' => 'Ressources Pratiques',
    'official_website' => 'Visiter le Site Officiel',
    'wikipedia_link' => 'Découvrir notre Histoire',
    'paris_city_guide' => 'Vivre à Paris : Spécial Étudiants',

    // Main Content
    'page_title' => 'Université Paris-Saclay : Votre Futur au Cœur de l\'Excellence Mondiale',
    'intro_content' => 'Si vous rêvez d\'un lieu où la curiosité rencontre l\'expertise mondiale, l\'Université Paris-Saclay vous ouvre ses portes. Fondée en 2019, c\'est une communauté vibrante où les plus grandes écoles et centres de recherche s\'unissent pour votre succès. Plus qu\'une université, c\'est un véritable tremplin pour votre carrière en sciences, santé ou ingénierie, au cœur de l\'innovation française.',

    'global_leader_title' => 'Une Éducation de Rang Mondial, pensée pour Vous',
    'global_leader_content' => 'En choisissant Paris-Saclay, vous rejoignez l\'élite académique mondiale. Pour 2026, notre université confirme son excellence :
- Classement de Shanghai 2026 : 13e mondiale (et fièrement n°1 en Europe continentale !).
- Classement QS 2026 : 70e mondiale.
- Times Higher Education 2026 : 64e mondiale.

Ces chiffres témoignent de la qualité de l\'accompagnement et des opportunités exceptionnelles qui vous attendent en tant qu\'étudiant ici.',

    'programs_title' => 'Trouvez la Voie qui correspond à vos Ambitions',
    'programs_content' => 'Quelle que soit votre passion, nous avons un programme adapté à vos objectifs. De vos premiers pas en Licence aux recherches avancées en Doctorat, nos facultés vous accompagnent dans votre spécialisation :',

    'subjects' => [
        'Sciences Naturelles et Fondamentales (Explorer l\'Univers)',
        'Sciences de la Vie et de la Santé (Inventer la Médecine du futur)',
        'Ingénierie et Technologies (Bâtir le Monde moderne)',
        'Sciences du Numérique et Data (Maîtriser l\'ère de l\'IA)',
        'Droit et Politiques Publiques (Votre carrière dans la Justice)',
        'Économie et Commerce International',
        'Management et Stratégie',
        'Sciences Sociales et Humaines',
    ],

    'research_title' => 'Rejoignez une Communauté de Pionniers',
    'research_content' => 'À Paris-Saclay, la recherche n\'est pas qu\'une affaire de laboratoire : vous en faites partie intégrante. Que vous soyez passionné par la physique quantique ou les enjeux climatiques, vous apprendrez aux côtés d\'experts qui changent le monde. C\'est un environnement stimulant où vos idées comptent vraiment.',

    'admission_title' => 'Nous rejoindre : Votre Feuille de Route 2026',
    'admission_content' => 'Nous savons que s\'installer dans un nouveau pays est une grande étape, et nous sommes là pour vous simplifier la vie. Pour 2026, préparez vos relevés de notes et votre certificat de français. Ne vous laissez pas impressionner par les démarches de visa : en anticipant et en suivant nos conseils, tout se passera bien. Nous avons hâte de vous aider à chaque étape de votre dossier !',

    'career_title' => 'Une Carrière sans Frontières',
    'career_content' => 'Un diplôme de Paris-Saclay est votre passeport pour une carrière internationale. Nous collaborons étroitement avec les leaders de l\'industrie mondiale. Une fois diplômé, ce n\'est pas seulement vous qui cherchez un emploi, ce sont les recruteurs qui viennent à vous. Que vous souhaitiez diriger une entreprise ou une équipe de recherche, nous vous offrons le réseau et le mentorat nécessaires.',

    'environment_title' => 'Votre Nouveau Chez-Vous',
    'environment_content' => 'Imaginez un campus où se croisent plus de 100 nationalités ! À Paris-Saclay, vous vivrez une aventure multiculturelle unique et vous ferez des amis pour la vie. Notre campus est pensé pour votre bien-être, avec :',

    'facilities' => [
        'Espaces d\'Innovation (Idéal pour le travail d\'équipe)',
        'Cafés et Restauration conviviale',
        'Théâtres et Ateliers artistiques',
        'Bibliothèques où l\'on se sent bien',
    ],

    'conclusion_title' => 'Votre Aventure commence ici',
    'conclusion_content' => 'Choisir Paris-Saclay, c\'est choisir un avenir plein de promesses. Vous ne venez pas seulement chercher un diplôme, vous rejoignez une famille d\'innovateurs et de leaders. Nous avons hâte de voir tout ce que vous allez accomplir. Faisons de vos rêves pour 2026 une réalité !',

    // Features List
    'features' => [
        'Diplôme d\'Excellence reconnu partout',
        'Laboratoires qui boostent la créativité',
        'Communauté Internationale soudée',
        'Lien direct avec les grands Groupes industriels',
        'Un Campus vivant, conçu pour les Étudiants',
    ],

    // Contact Form
    'ask_question' => 'Une Question ? Discutons-en !',
    'name_placeholder' => 'Comment vous appelez-vous ?',
    'email_placeholder' => 'Votre adresse e-mail favorite',
    'phone_placeholder' => 'La meilleure façon de vous joindre',
    'subject_placeholder' => 'Comment pouvons-nous vous aider ?',
    'message_placeholder' => 'Dites-nous tout sur vos projets ou vos questions...',
    'send_message' => 'Envoyer mon Message',
    'name_error' => 'N\'oubliez pas de nous donner votre nom !',
    'email_error' => 'Une adresse e-mail valide est nécessaire pour vous répondre.',
    'phone_error' => 'Laissez-nous un numéro pour vous rappeler.',
    'subject_error' => 'Précisez le sujet pour que nous trouvions le bon expert.',
    'message_error' => 'Veuillez rédiger votre message.',

    // JSON-LD Schema
    'schema_headline' => 'Université Paris-Saclay : Votre voyage vers 2026 commence ici',
    'schema_description' => 'Guide complet et bienveillant sur l\'Université Paris-Saclay (13e mondiale). Découvrez nos programmes et lancez votre candidature pour 2026.',
    'schema_author' => 'A.V.C - Votre Guide Personnel pour réussir en France',

    // FAQ Section
    'faq_title' => 'Vos Questions, nos Réponses',
    'faq_subtitle' => 'Conseils complices pour futurs étudiants',
    'faq_items' => [
        [
            'question' => 'Est-ce que Paris-Saclay est vraiment au top ?',
            'answer' => 'Absolument ! Classée 13e mondiale et 1ère en Europe continentale, l\'université offre des ressources et un soutien que vous ne trouverez nulle part ailleurs.',
        ],
        [
            'question' => 'Peut-on étudier en anglais ?',
            'answer' => 'Oui ! Si la Licence est surtout en français, nous proposons un très large choix de Masters et Doctorats intégralement en anglais, surtout en Sciences et Ingénierie.',
        ],
        [
            'question' => 'Comment est l\'ambiance de recherche ?',
            'answer' => 'C\'est passionnant. Nous réalisons 15 % de la recherche française. Vous apprendrez aux côtés de médaillés Fields et de Prix Nobel. C\'est le meilleur endroit pour apprendre des meilleurs.',
        ],
        [
            'question' => 'Quand postuler pour la rentrée 2026 ?',
            'answer' => 'Le plus tôt est le mieux ! Nous conseillons de commencer 6 à 9 mois à l\'avance pour avoir l\'esprit tranquille concernant le visa et les démarches.',
        ],
        [
            'question' => 'Est-ce facile de se loger ?',
            'answer' => 'Comme partout en région parisienne, il faut s\'y prendre tôt. Mais pas de panique : notre "Welcome Desk" et le CROUS sont là pour vous aider à trouver votre futur foyer.',
        ],
    ],
];
